...
- Les [champs d'application OAuth2](/docs/topics/OAauth2#shared-resources-oauth2-scopes) déterminent les accès aux données et actions que ton application peut effectuer. Elles sont accordées au nom de l'utilisateur qui procède à l'installation ou qui s'authentifie.
...
Tu peux consulter une liste de tous les [champs d'application OAuth2](/docs/topics/OAuth2#shared-resources-oauth2-scopes) ou en apprendre davantage sur les [permissions](/docs/topics/Permissions) dans la documentation.
...
Si tu souhaites personnaliser les commandes ou en ajouter d'autres, tu peux consulter la structure des commandes dans la [documentation des commandes](/docs/interactions/Application_Commands#application-command-object-application-command-structure).
...
Tu peux en apprendre davantage sur les options de commande et leur structure [en consultant la documentation](/docs/interactions/Application_Commands#application-command-object-application-command-option-structure).
...
Lorsque des utilisateurs interagissent avec un élément de message, Discord envoie une requête avec un [type d'interaction](/docs/interactions/Receiving_and_Responding#interaction-object-interaction-type) égal à `3` (ou la valeur `MESSAGE_COMPONENT` lors de l'utilisation de `discord-interactions`).
...
Le tableau `options` est peuplé en utilisant la méthode `getShuffledOptions()` dans `game.js`, qui manipule les valeurs `RPSChoices` pour être conforme au format des [options des éléments de message](/docs/interactions/Message_Components#select-menu-object-select-option-structure).

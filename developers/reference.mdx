...
You can filter for up to 10 file types in [File Upload](/developers/components/reference#file-upload-file-upload-structure) components and [Slash Command Options](/developers/interactions/application-commands#application-command-object-application-command-option-structure). Each value must be either one of the group names below, or a dot-prefixed file extension (e.g. `.pdf`).

<Note>
  If you specify dot-prefixed extensions instead, keep in mind that mobile clients may require additional variants for compatibility: include .jpg for image uploads, and both .mp4 and .mov for video uploads.
</Note>

---
title: How Do I Add Spatial Audio to My Game?
sidebarTitle: Add Spatial Audio
---
...
<img src="/images/game-development/how-to-add-spatial-audio-to-your-game/docs-spatial-audio.webp" alt="A banner showing a microphone that produces audio waves that look like they're in 3d space" style={{width: "100%", height: "auto"}} />

*Note: This guide was published on April XXth 2026 and was last updated on April XXth 2026.*

Spatial audio in multiplayer games has become a genre-defining feature. Hearing a friend's voice fade as they disappear around a corner, or listening to a teammate call out from across the map, makes a multiplayer game feel alive in a way that flat voice chat can’t. In this guide, you will learn how to combine the Discord Social SDK's voice chat with Unity's 3D audio system to build proximity voice chat for a multiplayer game. Discord handles everything about the voice call itself, and your game just needs to put the audio in the right place. While this guide focuses on Unity, the same concepts apply to any engine. You intercept the decoded audio from Discord and route it to your engine's 3D audio system.
...
[TODO: add preview vid of spatial audio]
...
Before diving into any code, it helps to understand the full architecture at a conceptual level. The spatial audio pipeline has five stages.
...
The Social SDK lets you start a voice call for a lobby, but here is the key difference from a normal voice call: instead of calling `StartCall()` and letting Discord handle playback through the user's default audio device, you call [`StartCallWithAudioCallbacks()`](/developers/docs/social-sdk/classdiscordpp_1_1Client.html#abcaa891769f9e912bfa0e06ff7221b05). This function intercepts the audio pipeline and gives you a callback that fires every time Discord has decoded audio ready for a player.

### 3. Intercepting the Audio

When Discord receives and decodes voice audio from a remote player, it calls your [`UserAudioReceivedCallback`](/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a49934aeb3ba5093c2362dedfd1e9ad3b). This callback hands you the raw PCM audio data along with the user ID of the speaker. It also gives you an `outShouldMute` flag. Setting this to `true` tells Discord not to play the audio through its normal output and allows you to control where it gets played.
...
Subscribe to `SetLobbyMemberAddedCallback()` to spawn the remote player and register them:
...
When a player joins a lobby, start a voice call using `StartCallWithAudioCallbacks`. Provide two callbacks: one for received audio (`OnVoiceAudioReceived` defined below), which you will use for spatial positioning, and one for outgoing audio, which you can leave empty since Discord handles microphone capture.
...
Voice Activity Detection (VAD) is how Discord determines whether a player is speaking or silent. Audio below the threshold is suppressed rather than transmitted. [`SetVADThreshold(false, -80f)`](/developers/docs/social-sdk/classdiscordpp_1_1Call.html#a7c3fd83c5dfe37d796e30c5e28c93b6e) sets a low threshold for voice detection (VAD). A low threshold like this allows players to whisper and still be heard. Tuning this value lower to `-100f` will allow all audio to come through but you may hear keyboard clicks and other noise. Raising it or removing this call will use a standard threshold set for regular volume speech.
...
{
		// This assumes a 48kHz sample rate for audio from Discord
    ...
    public void FeedSamples(IntPtr data, ulong samplesPerChannel, ulong channels)
    {
        ...
        lock (_lock)
        {
            for (int i = 0; i < (int)samplesPerChannel; i++)
            {
                ...
                {
                    mono += shorts[i * channelCount + c] / 32768f;
                }
                mono /= channelCount;
    ...
    private void OnPCMRead(float[] data)
    {
        lock (_lock)
        {
            ...
            if (available > TargetBufferSamples)
            {
                ...
                _readPosition = (_readPosition + excess) % RingBufferSamples;
...
<img src="/images/game-development/how-to-add-spatial-audio-to-your-game/unity-3d-audio.webp" alt="3D audio settings in the Unity AudioSource inspector" style={{width: "60%", height: "auto"}} />
...
1. A player joins lobby and `StartCallWithAudioCallbacks()` starts the Discord voice call with your custom audio callback.
...
[TODO: a fun spatial audio clip]

---

## Next Steps

Ready to go deeper? These guides cover the systems that work alongside spatial audio:
...
<Columns cols={3}>
    ...
    <Card title="Managing Lobbies" href="/developers/discord-social-sdk/development-guides/managing-lobbies" icon={<DoorEnterIcon />}>
        Create and manage multiplayer lobbies using the Discord Social SDK
    </Card>

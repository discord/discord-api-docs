...
Beyond the permission calculations above, `VIEW_CHANNEL` also governs what a bot can see about a channel. Over the Gateway, channels a bot lacks `VIEW_CHANNEL` on are dispatched with obfuscated data. [Get Guild Channels](/developers/resources/guild#get-guild-channels) will also omit those channels from its response entirely. This is currently true if the client has opted into the `CHANNEL_OBFUSCATION` [capability](/developers/events/gateway-events#identify-gateway-capabilities). Starting October 12, 2026, it will be true for all bots. See [Obfuscated Channels](/developers/resources/channel#channel-object-obfuscated-channels) for information.

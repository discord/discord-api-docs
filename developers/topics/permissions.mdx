...
Beyond the permission calculations above, `VIEW_CHANNEL` also governs what an app can see about a channel. Over the Gateway, channels an app lacks `VIEW_CHANNEL` on are dispatched with obfuscated data if the app has opted into the relevant capability. Starting October 12, 2026, [Get Guild Channels](/developers/resources/guild#get-guild-channels) will also omit those channels from its response entirely. See [Obfuscated Channels](/developers/resources/channel#channel-object-obfuscated-channels) for the redacted fields, how to handle them, and how to detect and surface hidden channels to server admins.

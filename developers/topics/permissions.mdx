...
Over the [Gateway](/developers/events/gateway), a client will receive a channel object with obfuscated metadata for guild channels it does not have permission to view. [Get Guild Channels](/developers/resources/guild#get-guild-channels) will also omit those channels from its response entirely. Permission to view a channel is defined by having `VIEW_CHANNEL` permission on a channel or by being connected to a voice a channel. Channel categories are viewable if any of their children channels are viewable.

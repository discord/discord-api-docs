...
<Collapsible title="RPC over WebSocket (Deprecated)" description="Legacy transport for older private beta participants." icon="warning">
<Danger>
...
| USER_ACHIEVEMENT_UPDATE                                                                       | sent when a user's achievement status is updated                                           |
...
| VOICE_SETTINGS_UPDATE_2                                                                       | updated version of voice settings dispatch                                                 |
...
| GAME_JOIN                                                                                     | legacy event for joining a game session                                                    |
| GAME_SPECTATE                                                                                 | legacy event for spectating a game session                                                 |
| LOBBY_DELETE                                                                                  | sent when a joined lobby is deleted                                                        |
| LOBBY_UPDATE                                                                                  | sent when lobby metadata or settings change                                                |
| LOBBY_MEMBER_CONNECT                                                                          | sent when a new member joins the lobby                                                     |
| LOBBY_MEMBER_DISCONNECT                                                                       | sent when a member leaves the lobby                                                        |
| LOBBY_MEMBER_UPDATE                                                                           | sent when a lobby member's metadata changes                                                |
| LOBBY_MESSAGE                                                                                 | sent when a message is received in the lobby's networking layer                            |
| OVERLAY                                                                                       | sent when the Discord overlay is toggled or initialized                                    |
| OVERLAY_UPDATE                                                                                | sent when the overlay's state or configuration changes                                     |
| CAPTURE_SHORTCUT_CHANGE                                                                       | sent when the user changes a registered global hotkey                                      |

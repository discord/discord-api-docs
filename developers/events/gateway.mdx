...
Clients can also opt into a [gateway capability](/developers/events/gateway-events#identify-gateway-capabilities) in the [Identify payload](/developers/events/gateway-events#identify-identify-structure) that changes how existing gateway behaviors work.
...
Because your guilds are spread across `max_concurrency` shards, [Get Current User Guilds](/developers/resources/user#get-current-user-guilds) requires the `shard` query parameter to be specified. Request each shard from `0` to `max_concurrency - 1` separately to retrieve all of your guilds. Note that `shard` in this context is not the same as the shard you use to connect to the Gateway.
...
| session_start_limit | [session_start_limit](/developers/events/gateway#session-start-limit-object) object | Information on the current session start limit                                             |

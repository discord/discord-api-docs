...
Because your guilds are stored across `max_concurrency` shards, [Get Current User Guilds](/developers/resources/user#get-current-user-guilds) requires the `shard` query param to be specified. Request each shard from `0` to `max_concurrency - 1` separately to retrieve all of your guilds.

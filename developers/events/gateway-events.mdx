...
| capabilities?    | integer                                                                     | Opt-in bitfield representing [capabilities](/developers/events/gateway-events#identify-gateway-capabilities) of your gateway client | 0       |

<ManualAnchor id="identify-gateway-capabilities" />
###### Gateway Capabilities

`capabilities` is a bitfield you can use to opt an app into gateway behaviors. It's a separate Identify bitfield from [`intents`](/developers/events/gateway#gateway-intents): `intents` control which events your app receives, while `capabilities` change how existing gateway behaviors work.

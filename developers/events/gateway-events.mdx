...
| capabilities?    | integer                                                                     | [gateway capabilities](/developers/events/gateway-events#identify-gateway-capabilities) opt-in bitfield for gateway behaviors  | 0       |

<ManualAnchor id="identify-gateway-capabilities" />
###### Gateway Capabilities

`capabilities` is a bitfield you can use to opt an app into experimental or in-progress gateway behaviors. It's a separate Identify bitfield from [`intents`](/developers/events/gateway#gateway-intents): `intents` control which events your app receives, while `capabilities` change how existing gateway behaviors work.

| Flag                | Value     | Description                                                                                                                                                             |
|---------------------|-----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CHANNEL_OBFUSCATION | `1 << 15` | Opts the app into receiving [obfuscated channel metadata](/developers/resources/channel#channel-object-obfuscated-channels) over the Gateway for channels it can't view |

<Warning>
`CHANNEL_OBFUSCATION` is a temporary, testing-only opt-in for channel obfuscation. This opt-in mechanism will change before the feature reaches general availability. Obfuscation is then planned to apply to all apps automatically, without requiring this capability. Don't add support for this capability to libraries.
</Warning>

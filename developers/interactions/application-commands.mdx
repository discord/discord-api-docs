...
| file_types?\*\*            | array of strings                                                                                                                                                   | File types to filter for; can be `image`, `video`, `audio` or any dot-prefixed extension such as `.pdf`; max 10                           | `ATTACHMENT`                                  |
...
\*\*\* `file_types` will only check the extension on the filename. Check [File Groups](/developers/reference#file-groups) for more details.

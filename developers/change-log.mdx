...
- **Over the Gateway**, channels a bot can't view will still be dispatched, but with sensitive fields redacted. The channel's `name` becomes `"___hidden___"`, other sensitive fields (`topic`, `parent_id`, `permission_overwrites`, and more) are nulled or reduced, and a new [`CHANNEL_OBFUSCATED` flag](/developers/resources/channel#channel-object-channel-flags) (`1 << 17`) is set on the channel's `flags`.

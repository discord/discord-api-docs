...
<Update label="September 28, 2026" tags={["Docs"]} rss={{
    title: "Videos in the Documentation",
    description: "Videos from the Discord Developers YouTube channel are now embedded throughout the docs, with chapter navigation and running times on long-form content."
}}>

    ## Videos in the Documentation

    Like to watch instead of read? Amazing — we've embedded videos from the [Discord Developers](https://youtube.com/@discorddevelopers) YouTube channel directly into the documentation, right next to the guides they pair with.

    Long-form videos show their total running time and come with a chapter list, so you can jump straight to the section you need without leaving the page.

    You'll find them throughout the docs, including on:

    - [Provisional Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/overview)
    - [Application Commands](/developers/interactions/application-commands)
    - [Integrate Moderation](/developers/discord-social-sdk/how-to/integrate-moderation)
    - [Add Proximity Voice Chat to Your Game](/developers/game-development/how-to-add-proximity-voice-chat-to-your-game)

    Keep an eye out for them across the platform documentation! We'll be adding more as we release more content!
</Update>

<Update label="September 22, 2026" tags={["Gateway", "Activities", "Premium Apps", "Discord Social SDK"]} rss={{
    title: "Age Assurance: What It Means for Discord Developers",
    description: "Age assurance is rolling out globally. Apps continue to work as is, but you may observe changes to custom status visibility, age-restricted Activities, parent-set spending limits, and DM filtering."
  }}>

  ## Age Assurance: What It Means for Discord Developers

  We [recently announced the launch of age assurance](https://discord.com/blog/safer-for-teens-same-discord-for-adults). For most developers, nothing changes: existing apps, bots, games, Activities, and Social SDK integrations continue to work as is.
  ...
  - **Bots & Apps:** Private Profiles limit the visibility of custom statuses from user presence.
  - **Activities:** Age-restricted Activities may require users to confirm their age group.
  - **Commerce:** Parent-set spending limits may prevent some purchases.
  - **Social SDK:** DMs may be filtered to Message Requests more often.

  Full details are available in our [help center article](https://support-dev.discord.com/hc/en-us/articles/38338398970263-Age-Assurance-What-Developers-Need-to-Know). If you hit something unexpected, please open a ticket with Developer Support.
</Update>

<Update label="September 18, 2026" tags={["HTTP API"]} rss={{
    title: "New Invite Target User Endpoints and Options",
    description: "New endpoints and options to create, add, and remove individual or multiple target users on an invite without using a CSV."
  }}>

  ## New Invite Target User Endpoints and Options

  [Create Channel Invite](/developers/resources/channel#create-channel-invite) now takes an array of target user IDs without uploading a CSV file by using `target_user_ids`. [Update Target Users](/developers/resources/invite#update-target-users) replaces an invite's entire target user list from an uploaded CSV, which is processed as an asynchronous job. Changing a single user meant rebuilding and re-uploading the whole file. These four endpoints adjust the list in place, extending the [invite endpoints](/developers/change-log#new-invite-endpoints).

  - [Add Target User](/developers/resources/invite#add-target-user): Adds a single user to an invite's target users
  - [Remove Target User](/developers/resources/invite#remove-target-user): Removes a single user from an invite's target users
  - [Bulk-Add Target Users](/developers/resources/invite#bulk-add-target-users): Adds multiple users, passed as a `user_ids` array of snowflakes
  - [Bulk-Delete Target Users](/developers/resources/invite#bulk-delete-target-users): Removes multiple users, passed as a `user_ids` array of snowflakes

  All four require the caller to be the inviter or have the `MANAGE_GUILD` permission.
</Update>

<Update label="September 17, 2026" tags={["Gateway"]} rss={{
    title: "Custom Status Hidden by Profile Privacy Settings",
    description: "The custom status activity is omitted from Presence Update events dispatched to a guild based on the presence author's profile privacy setting."
  }}>

  ## Custom Status Hidden by Profile Privacy Settings

  If a user's [Profile Privacy Setting](https://support.discord.com/hc/en-us/articles/38859942749463-Profile-Privacy-Setting-on-Discord) is set to `Friends Only`, or to `Friends & Small Servers Only` in a guild with more than 200 members, their custom status (an [activity object](/developers/events/gateway-events#activity-object) with `type` `4`) is omitted from the `activities` field of [Presence Update](/developers/events/gateway-events#presence-update) events dispatched to that guild.

  This applies to apps and bots subscribed to the guild's presence. Other activity types (Playing, Streaming, Listening, Watching, Competing) are not affected by this setting but may be controlled by [Activity Sharing settings](https://support.discord.com/hc/en-us/articles/7931156448919-Activity-Sharing-on-Discord-FAQ).
</Update>

<Update label="September 17, 2026" tags={["Discord Social SDK", "HTTP API"]} rss={{
    title: "Unlinking Accounts Guide",
    description: "New guide covering best practices for unlinking a user's Discord account, including when to revoke tokens and how to avoid relink conflicts."
  }}>

  ## Unlinking Accounts Guide

  We've published a new guide, [Unlinking Accounts](/developers/discord-social-sdk/development-guides/unlinking-accounts), covering best practices for unlinking a user's Discord account from your game.

  The guide explains why you should always revoke a user's access and refresh tokens when unlinking rather than only removing your own local record, what can go wrong if you don't (including relinking conflicts), and how to handle out-of-band unlinks using the [`APPLICATION_DEAUTHORIZED`](/developers/events/webhook-events#application-deauthorized) webhook event.
</Update>

<Update label="September 15, 2026" tags={["HTTP API", "Breaking Change"]} rss={{
    title: "Shard Param Required for Get Current User Guilds with Large Bot Sharding",
    description: "Apps using large bot sharding must pass the shard query param to Get Current User Guilds."
  }}>

  ## Shard Param Required for Get Current User Guilds with Large Bot Sharding

  [Get Current User Guilds](/developers/resources/user#get-current-user-guilds) accepts a `shard` query param, which returns only the guilds stored in that shard.

  Apps using [large bot sharding](/developers/events/gateway#sharding-for-large-bots) are **required** to pass it — requests without `shard` now return `400 Bad Request`. Valid values are `0` through `max_concurrency - 1`, using the `max_concurrency` returned by [Get Gateway Bot](/developers/events/gateway#get-gateway-bot),

  Apps that are not using large bot sharding are unaffected.
</Update>
...
<Update label="September 3, 2026" tags={["HTTP API", "Interactions"]} rss={{
  ...
  ## Default File Upload Limit Increase
...
<Update label="September 1, 2026" tags={["Discord Social SDK"]} rss={{
    title: "Discord Social SDK Release 1.10.19337",
    description: "Read-only access to the Discord desktop client's voice settings — mute/deafen, volumes, voice mode, and push-to-talk key — over RPC."
  }}>

    ## Discord Social SDK Release 1.10.19337

    A new release of the Discord Social SDK is now available, with the following updates:

    ### Voice
    Added read-only access to the Discord desktop client's voice settings over RPC, including mute/deafen state, volumes, voice mode, and the configured push-to-talk key.

    Fetch the current settings with [`Client::GetVoiceSettings`](https://discord.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a3ca1d435f1a2861462e2b3d313b15aec), or subscribe to changes with [`Client::SetVoiceSettingsUpdatedCallback`](https://discord.com/developers/docs/social-sdk/classdiscordpp_1_1Client.html#a2006d6b52b1665fb51172a98c3a37661) — both return the new [`VoiceSettings`](https://discord.com/developers/docs/social-sdk/classdiscordpp_1_1VoiceSettings.html) class.

    See [Reading Voice Settings from the Discord Client](/developers/discord-social-sdk/development-guides/managing-voice-chat#reading-voice-settings-from-the-discord-client) for details.
</Update>

<Update label="August 27, 2026" tags={["HTTP API", "Breaking Change"]} rss={{
...
<Update label="August 26, 2026" tags={["Discord Social SDK", "HTTP API"]} rss={{
    title: "Game Stats Widgets",
    description: "Claimed games can now display player stats like rank, playtime, and wins on Discord user profiles using the new Application Identity Profile API."
  }}>

  ## Game Stats Widgets

  Games that have been [claimed on Discord](https://docs.discord.com/developers/platform/claim-your-game) can now configure a [Game Stats Widget](/developers/social-layer/game-stats-widgets/overview) to show off player progress, such as rank, playtime, wins, and any custom stat you define, directly on their Discord profile.

  - Configure widget layouts and fields in the Developer Portal
  - Populate pre-defined **primary** fields (`rank_name`, `total_wins`, `playtime_hours`, etc.) or your own **dynamic** fields using your bot token
  - Data is written to a user's [Application Identity Profile](/developers/resources/application-identity-profile), a record of the user's identity in your game

  See the [Game Stats Widgets overview](/developers/social-layer/game-stats-widgets/overview) to get started.
</Update>
...
- **Over the Gateway**, channels a bot can't view will still be dispatched, but with sensitive fields obfuscated. The channel's `name` becomes `"___hidden___"`, other sensitive fields are nulled or reduced, a new [`CHANNEL_OBFUSCATED` flag](/developers/resources/channel#channel-object-channel-flags) (`1 << 17`) is set on the channel's `flags`, and the `permission_overwrites` will contain a single permission overwrite denying `VIEW_CHANNEL` for the guild's `@everyone` role.
- **Over HTTP**, [`GET /guilds/{guild.id}/channels`](/developers/resources/guild#get-guild-channels) will omit those channels from the response entirely.

A channel stops being obfuscated the moment your bot gains access to it, for example through a permission overwrite change or a role change. When that happens, the Gateway dispatches a [Channel Update](/developers/events/gateway-events#channel-update) event with the channel's full, unobfuscated data. [Interaction](/developers/interactions/receiving-and-responding) payloads are built through a separate path that does not apply obfuscation.
...
If your bot manages or organizes channels, see [Obfuscated Channels](/developers/resources/channel#channel-object-obfuscated-channels) for how to detect channels your bot can't see and surface that to users.

### What Can You Do to Test This Change?

<img src="/images/dev-portal/private-channel-obfuscation.webp" alt="Dev portal UI showing the opt-in toggle for private channel obfuscation" style={{width: "100%", height: "auto"}} />

- **Gateway:** If your bot interacts with channels, you can test the obfuscation behavior today by sending `1 << 15` in the `capabilities` field of your [Identify](/developers/events/gateway-events#identify-gateway-capabilities) payload. You can also opt in through the [developer portal](https://discord.com/developers/applications/select/bot) by enabling the **Private Channel Obfuscation** toggle in the **Overview > Bot** tab. This capability and the toggle are temporary and for testing.
...
<Update label="August 5, 2026" tags={["HTTP API", "Breaking Change"]} rss={{
    ...
    The `application_id` field on the [channel](/developers/resources/channel#channel-object) object is now nullable. For future guild channels, this field can be serialized as `null`. For group DMs, there are no plans to serialize this field as `null`.
...
<Update label="July 22, 2026" tags={["Discord Social SDK"]} rss={{
    ...
    The single **Using Provisional Accounts** guide has been reorganized into a dedicated [Provisional Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/overview) section, with a focused page for each part of the flow:

    - [Overview](/developers/discord-social-sdk/development-guides/provisional-accounts/overview) — core concepts, the account lifecycle, and how to choose an authentication method.
    - Creating accounts: [Bot Token Endpoint](/developers/discord-social-sdk/development-guides/provisional-accounts/bot-token-endpoint), [External Credentials Exchange](/developers/discord-social-sdk/development-guides/provisional-accounts/external-credentials-exchange), and [Public Client Integration](/developers/discord-social-sdk/development-guides/provisional-accounts/public-client).
    - Reference and lifecycle: [Configuring Identity Providers](/developers/discord-social-sdk/development-guides/provisional-accounts/identity-providers), [Managing Provisional Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/managing-accounts), [Merging Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/merging-accounts), and [Unmerging Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/unmerging-accounts).

    The [Bot Token Endpoint](/developers/discord-social-sdk/development-guides/provisional-accounts/bot-token-endpoint) and [Merging Accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/merging-accounts) guides now walk through the full client-to-server integration, with sequence diagrams for each flow.
</Update>

<Update label="July 16, 2026" tags={["Interactions"]} rss={{
    title: "Resolved Channel app_permissions in Interactions",
    description: "Resolved channels in interactions now include the app_permissions field."
  }}>
...
<Update label="May 22, 2026" tags={["Discord Social SDK"]} rss={{
  ...
  - **[Ban-Driven Unmerge](/developers/discord-social-sdk/development-guides/provisional-accounts/unmerging-accounts#ban-driven-unmerge)** documents the full lifecycle when a user's Discord account is banned: tokens are deleted immediately, a new cross-platform-restricted provisional account is created carrying the original external identity, and merge attempts return error [`530017`](/developers/discord-social-sdk/development-guides/provisional-accounts/merging-accounts#merge-request-failures) until the ban lifts (or, for permanent bans, indefinitely).
...
<Update label="April 22, 2026" tags={["Discord Social SDK"]} rss={{
  ...
  We've made security improvements to how Discord validates OIDC issuer URLs and fetches OIDC configuration for [provisional accounts](/developers/discord-social-sdk/development-guides/provisional-accounts/overview) — including stricter hostname checks and blocking HTTP redirects during configuration and JWKS discovery.
...
<Update label="April 16, 2026" tags={["Discord Social SDK"]} rss={{
  ...
  ## Deprecation: OAuth2 Refresh Token Grant for OIDC Provisional Accounts
    ...
    If you are using [OIDC server-side authentication](/developers/discord-social-sdk/development-guides/provisional-accounts/external-credentials-exchange#server-authentication-with-external-credentials-exchange)
    ...
    See the [Refreshing Access Tokens](/developers/discord-social-sdk/development-guides/provisional-accounts/managing-accounts#refreshing-access-tokens)
    section for details.
...
<Update label="August 20, 2025" tags={["Breaking Change", "HTTP API"]} rss={{
...
<Update label="December 16, 2024" tags={["HTTP API", "Interactions", "Breaking Change"]} rss={{
...
<Update label="August 28, 2024" tags={["Premium Apps", "Breaking Change"]} rss={{
...
<Update label="July 17, 2024" tags={["Activities", "Embedded App SDK", "Breaking Change"]} rss={{
...
<Update label="June 27, 2024" tags={["Breaking Change"]} rss={{
...
<Update label="March 15, 2024" tags={["Breaking Change"]} rss={{
...
<Update label="January 09, 2023" tags={["Breaking Change"]} rss={{
...
<Update label="November 17, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="November 09, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="September 01, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="July 01, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="June 29, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="June 21, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="April 27, 2022" tags={["Breaking Change"]} rss={{
...
<Update label="January 03, 2018" tags={["Breaking Change"]} rss={{
...
<Update label="October 16, 2017" tags={["Breaking Change"]} rss={{
...
<Update label="August 16, 2017" tags={["Breaking Change"]} rss={{
...
<Update label="August 03, 2017" tags={["Breaking Change"]} rss={{
...
<Update label="July 19, 2017" tags={["Breaking Change"]} rss={{

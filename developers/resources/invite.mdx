...
Gets the users allowed to see and accept this invite. Response is a CSV file containing the user IDs in the same format file that was uploaded on [invite create](/developers/resources/channel#create-channel-invite). Requires the caller to be the inviter, or have `MANAGE_GUILD` permission, or have `VIEW_AUDIT_LOG` permission.

...
| premium_type?           | integer                                                                                                            | the [type of Nitro subscription](/developers/resources/user#user-object-premium-types) on a user's account | identify.premium      |
...
| display_name_styles?    | ?[display name styles](/developers/resources/user#display-name-styles-object-display-name-styles-structure) object | styles applied to the user's display name                                                                  | identify              |
...
How a user's name gets displayed, such as font, colors, gradient, and glow.
...
| Field     | Type           | Description                                                                                                          |
| --------- | -------------- | -------------------------------------------------------------------------------------------------------------------- |
| font_id   | integer        | the [font](/developers/resources/user#display-name-styles-object-display-name-font) of the display name              |
| effect_id | integer        | the [effect](/developers/resources/user#display-name-styles-object-display-name-effect) applied to the display name  |
| colors    | array[integer] | the colors encoded as an array of integers representing hexadecimal color codes (max 2)                              |

<ManualAnchor id="display-name-styles-object-display-name-font" />
###### Display Name Font
...
<ManualAnchor id="display-name-styles-object-display-name-effect" />
###### Display Name Effect
...
<Warning>
Apps using [large bot sharding](/developers/events/gateway#sharding-for-large-bots) must provide the `shard` param. Valid values are `0` through `max_concurrency - 1`, where `max_concurrency` comes from the [session start limit object](/developers/events/gateway#session-start-limit-object) on [Get Gateway Bot](/developers/events/gateway#get-gateway-bot). Iterate over every shard to retrieve the full guild list. Apps that are not using large bot sharding are unaffected.

Note that this value is different from the shard id used when connecting to the [Gateway](/developers/events/gateway).
</Warning>

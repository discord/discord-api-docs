...
<Warning>
Apps using [large bot sharding](/developers/events/gateway#sharding-for-large-bots) must provide the `shard` param — a request without it returns `400 Bad Request`. Valid values are `0` through `max_concurrency - 1`, where `max_concurrency` comes from the [session start limit object](/developers/events/gateway#session-start-limit-object) on [Get Gateway Bot](/developers/events/gateway#get-gateway-bot); fetch one shard at a time to get every guild. Apps that are not using large bot sharding have a `max_concurrency` of `1` and are unaffected.
</Warning>

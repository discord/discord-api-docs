...
Over the [Gateway](/developers/events/gateway), a client will receive a channel object with obfuscated metadata for guild channels it does not have `VIEW_CHANNEL` permission for (and for voice channels it is not currently connected to), instead of the channel being omitted. Currently, this will only be true for clients that opt into the `CHANNEL_OBFUSCATION` [gateway capability](/developers/events/gateway-events#identify-gateway-capabilities) for testing, but starting October 12th 2026 all clients will recieve obfuscated channels for channels it does not have `VIEW_CHANNEL` permission for.

...
Set a voice channel's status. Requires the `SET_VOICE_CHANNEL_STATUS` permission, and additionally the `MANAGE_CHANNELS` permission if the current user is not connected to the voice channel. Returns `204 No Content` on success. Fires a [Voice Channel Status Update](/developers/events/gateway-events#voice-channel-status-update) Gateway event.

...
| CHANNEL_OBFUSCATED          | `1 << 17` | this channel's metadata has been obfuscated because the current app cannot view it. Only ever set on channels received over the [Gateway](/developers/events/gateway); the HTTP API never sets this flag. See [Obfuscated Channels](/developers/resources/channel#channel-object-obfuscated-channels). |

<ManualAnchor id="channel-object-obfuscated-channels" />
###### Obfuscated Channels

Over the [Gateway](/developers/events/gateway), an app that has opted into the `CHANNEL_OBFUSCATION` [gateway capability](/developers/events/gateway-events#identify-gateway-capabilities) will still receive a channel object for guild channels it does not have `VIEW_CHANNEL` permission for (and for voice channels it is not currently connected to), instead of the channel being omitted. Before the object is sent, most of its metadata is redacted.
...
To reliably detect whether a channel object has been obfuscated, check whether its `flags` field has the `CHANNEL_OBFUSCATED` [channel flag](/developers/resources/channel#channel-object-channel-flags) (`1 << 17`) set. Do not rely on inspecting `name` or any other redacted field.

<Info>
`id`, `type`, and `position` won't be redacted, even on an obfuscated channel. Your app can rely on their real values to reorder channels or identify and report that it is missing permissions. Other fields should not be relied on.
</Info>

When an app regains visibility into a previously obfuscated channel (for example, a permission overwrite change, a change to the app's roles, a change to a role's permissions, or the app joining a voice channel it previously couldn't see), the Gateway dispatches a [Channel Update](/developers/events/gateway-events#channel-update) event containing the full, unobfuscated channel data. If the channel's parent category was also obfuscated (because the app couldn't see it either), that category also receives a Channel Update event with its full, unobfuscated data.
...
<Tip>
To build a good experience around obfuscated channels:
- Check the `CHANNEL_OBFUSCATED` flag before rendering a channel's `name` or other redacted metadata in any UI.
- Give users a way to see which channels your app needs access to but currently can't see, such as a "channels this bot can't access" report or command.
- When onboarding fails because a channel is hidden, point the user at the exact permission to grant (`VIEW_CHANNEL` on the affected channel or its parent category)
</Tip>

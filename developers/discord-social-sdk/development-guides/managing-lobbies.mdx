...
The first time a user sends a message in a lobby that's linked to a Discord channel, the SDK will inject a disclosure
message into the chat that contains a basic English explanation that their messages are now visible on Discord. This
message is private to the sending user and does not appear in the lobby's message history—other lobby members will not see it.

You can identify these messages with the [`MessageHandle::DisclosureType`] method.

We encourage you to customize the rendering of these messages, possibly changing the wording, translating them,
and making them look aligned with the style and aesthetic of your game. You can choose to avoid rendering these as well.

This does not apply to lobbies that aren't linked to a Discord channel.
...
| Date              | Changes                                |
|-------------------|----------------------------------------|
| September 9, 2026 | Document legal disclosure message type |

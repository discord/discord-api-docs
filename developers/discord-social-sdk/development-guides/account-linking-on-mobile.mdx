...
<Info>
As of Discord Social SDK 1.5, mobile account linking has been significantly simplified through native deep-link authentication support. If the Discord mobile app is installed and the user is already signed in, this native flow reuses their existing authenticated Discord app session, so they won't need to re-enter credentials in a browser.
</Info>

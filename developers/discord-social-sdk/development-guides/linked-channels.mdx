...
This gives your players a persistent, cross-platform home for their lobby's conversation that survives after they log off, and it can help grow your Discord community by letting players join your server directly from the game (see [Joining Discord Servers via Linked Lobbies](#joining-discord-servers-via-linked-lobbies)).
...
2. Enable the `CanLinkLobby` flag (`1 << 0`) on your lobby member by either sending a request to the `/lobbies/<lobby_id>/members/<user_id>` endpoint or by including the member data in the body of the [Create Lobby](/developers/resources/lobby#create-lobby) request.
...
| Date              | Changes                               |
|-------------------|---------------------------------------|
| September 8, 2026 | Explain why you'd use Linked Channels |

...
![An image showing the different button styles](button-styles.webp)
...
![Example showing one primary button per button group](multiple-buttons-example-1.webp)

If there are multiple buttons of equal significance, use the `Secondary` button style for all buttons

![Example showing multiple buttons in a group with equal significance](multiple-buttons-example-2.webp)
...
![A premium button](premium-button.webp)
...
![A role select component on desktop](desktop-role-select-menu.webp)
...
![A text input in a modal on desktop client](modal-desktop.webp)

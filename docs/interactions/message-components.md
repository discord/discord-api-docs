...
Components are a field on the [message object](/resources/channel#message-object), so you can use them whether you're sending messages or responding to a [slash command](/interactions/application-commands) or other interaction.
...
Buttons are interactive components that render in messages. They can be clicked by users, and send an [interaction](/interactions/receiving-and-responding#interaction-object) to your app when clicked.
...
| style      | integer                                        | A [button style](/interactions/message-components#button-styles) |
...
There are 5 different [select menu components](/interactions/message-components#component-types) that can be included in Action Rows.

The string select menu (type `3`) is the *only* select type that allows (and *requires*) apps to define the `options` that appear in the dropdown list. The other 4 select menu components (users, roles, mentionables, and channels) are auto-populated with options corresponding to the resource type—similar to [command option types](/interactions/application-commands#application-command-option-type).

In addition to the `values` array in all [select menu interaction payloads](/interactions/message-components#select-menu-interaction), auto-populated select menu components (user, role, mentionable, and channel) also include an additional [`resolved` object](/interactions/message-components#select-menu-resolved-object) that provides additional details about the user's selected resource.

The payloads for the select menu components are detailed in the [select menu structure](/interactions/message-components#select-menu-structure).
...
| Field                 | Type                                                                                              | Description                                                                                                                                              |
|-----------------------|---------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| type                  | integer                                                                                           | [Type](/interactions/message-components#component-types) of select menu component (text: `3`, user: `5`, role: `6`, mentionable: `7`, channels: `8`)     |
| custom_id             | string                                                                                            | ID for the select menu; max 100 characters                                                                                                               |
| options?\*            | array of [select options](/interactions/message-components#select-option-structure)               | Specified choices in a select menu (only required and available for string selects (type `3`); max 25                                                    |
| channel_types?\*\*    | array of [channel types](/resources/channel#channel-types)                                        | List of channel types to include in the channel select component (type `8`)                                                                              |
| placeholder?          | string                                                                                            | Placeholder text if nothing is selected; max 150 characters                                                                                              |
| default_values?\*\*\* | array of [default value objects](/interactions/message-components#select-default-value-structure) | List of default values for auto-populated select menu components; number of default values must be in the range defined by `min_values` and `max_values` |
...
\*\*\* `default_values` is only available for auto-populated select menu components, which include user (`5`), role (`6`), mentionable (`7`), and channel (`8`) [components](/interactions/message-components#component-types).
...
When defining a text input component, you can set attributes to customize the behavior and appearance of it. However, not all attributes will be returned in the [text input interaction payload](/interactions/message-components#text-input-interaction).
...
| style        | integer | The [Text Input Style](/interactions/message-components#text-input-styles) |

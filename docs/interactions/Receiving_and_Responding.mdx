...
When responding to an interaction received, you can make a `POST` request to `/interactions/<interaction_id>/<interaction_token>/callback`. `interaction_id` is the unique id of that individual Interaction from the received payload. `interaction_token` is the unique token for that interaction from the received payload. **If you send this request for an interaction received over HTTP, respond to the original HTTP request with a 202 and no body.**
...
<Collapsible title="Inline HTTP Reponse Behavior" icon="code">
    > warn
    > The documentation in this section is a discouraged way of responding to interactions now. We highly recommend using the [HTTP callback method](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-response-object-callback) to receive results and any potential error.
...
Create a response to an Interaction. Body is an [interaction response](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-response-object). Returns `200` [interaction response](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-object).

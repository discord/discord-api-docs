...
| LAUNCH_ACTIVITY\*\*\*\*                 | 12    | launch the activity associated with the application                                                                                                                                                    |
...
\*\*\*\* Requires the application to have enable Activities. [Learn more about building Activities on Discord](#DOCS_ACTIVITIES_DEVELOPMENT_GUIDES/overview).
...
<Collapsible title="Inline HTTP Reponse Behavior" icon="code">
    > warn
    > The documentation in this section is a discouraged way of responding to interactions now. We highly recommend using the [HTTP callback method](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-callback) to receive results and any potential error.
...
| resource?   | [interaction resource object](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-response-object-interaction-callback-resource-object) | The resource that was created by the interaction response. |
...
| activity_instance_id?       | string    | The instance id of the activity if one was launched or joined                                       |
| response_message_id?        | snowflake | The id of the message that was created by the interaction                                           |
| response_message_loading?   | boolean   | Whether or not the mesage is in a loading state                                                     |
...
| activity_instance?\* | [Activity instance resource](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/interaction-response-object-interaction-callback-activity-instance-resource) | Represents the activity launched by this interaction.                                                                          |
| message?\*\*         | [message object](#DOCS_RESOURCES_MESSAGE/message-object)                                                                                              | The message created by the interaction.                                                                                        |
...
| Field | Type   | Description                                                    |
|-------|--------|----------------------------------------------------------------|
| id    | string | The instance id of the activity if one was launched or joined. |
...
| Field          | Type                                             | Description                                                                           |
|----------------|--------------------------------------------------|---------------------------------------------------------------------------------------|
| with_response? | [boolean](#DOCS_REFERENCE/boolean-query-strings) | Whether to include a [interaction callback response] as the response instead of a 204 |

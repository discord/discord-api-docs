...
[Register a `PRIMARY_ENTRY_POINT` app command](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-object-application-command-option-type). For new apps that go through the Activities-Getting Started flow, a new app command with type `PRIMARY_ENTRY_POINT` and handler `DISCORD_LAUNCH_ACTIVITY` will automatically get created. For apps that went through the flow during the Activities developer preview, you will need to manually register the app command.

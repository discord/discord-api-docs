...
- **Create a new Entry Point command** (if you don't already have one) using [creating a global command](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/making-a-global-command) and setting the [command `type`](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-object-application-command-types) to `PRIMARY_ENTRY_POINT` (type `4`).

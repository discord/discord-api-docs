...
#### Activity Supported Ports
When an activity makes a request (i.e. to https://<app_id>.discordsays.com), url port usage is limited to Cloudflare's list of supported ports as shown [here](https://developers.cloudflare.com/fundamentals/reference/network-ports/#network-ports-compatible-with-cloudflares-proxy).

...
Rich Presence data can be thought of as an extension of your Activity—and leveling it up just a *little* bit makes it more interesting and relevant to the user playing your Activity and their friends that could be compelled to jump in and play along with them. This guide provides an overview of the platform and technical knowledge you need to integrate Rich Presence with your existing Activity (with sample code provided!).
...
By default, when a user is connected to your Activity, the app name icon will appear on their profile. If the user viewing the profile has the ability to join the Activity, a "Join Activity" button will be displayed as well.

![Example of default Rich Presence data for an Activity](default-presence-activities.png)

While this is okay, it's really limited and doesn't provide much context about what a user is doing inside of the game. In the following sections, we'll take a look at what richer and more actionable Rich Presence can look like (complete with code samples).

### Custom Rich Presence Data

Now let's take a look at what custom Rich Presence data you can set when setting a user's presence. The [types for these fields](#DOCS_RICH_PRESENCE_USING_THE_EMBEDDED_APP_SDK/setactivity-fields) and [examples](#DOCS_RICH_PRESENCE_USING_THE_EMBEDDED_APP_SDK/example) are in the sections below, but for now let's just get an idea of what we're working with:

![Image of where Rich Presence data appears in Discord profiles for Activities](annotated-presence-data-activities.png)

Three small things to note about the image above:
1. `large_image` and `small_image` are both in the `assets` object, which you can see below in the [table below](#DOCS_RICH_PRESENCE_USING_THE_EMBEDDED_APP_SDK/activity-partial-object). They're labeled with the keys so it's more clear how they appear on a Discord profile.
...
3. Depending on the `type` you set when updating presence, the behavior and layout for some fields (like `small_image` and `large_text`) may slightly change.

## Updating Presence

When updating Rich Presence data using the Embedded App SDK, the only real command you need to use is **[`setActivity()`](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/setactivity)**. Under the hood, `setActivity()` calls the RPC [`SET_ACTIVITY` command](#DOCS_TOPICS_RPC/setactivity) with only the features and fields available when you're building an Activity with the Embedded App SDK.
...
As you start exploring the Rich Presence docs, you'll start seeing the word "activity" a *lot*. The "activities" referenced in docs like the [RPC docs](#DOCS_TOPICS_RPC/setactivity) aren't related to the Activities you're building with the Embedded App SDK.

When Rich Presence was introduced, the underlying object that contains all of the Rich Presence data was called an "activity" (long before the Embedded App SDK). That's the sort of "activity" the RPC [`SET_ACTIVITY` command](#DOCS_TOPICS_RPC/setactivity) (and the [containing `activity` object](#DOCS_TOPICS_GATEWAY_EVENTS/activity-object)) is referencing. And that's also why the Embedded App SDK's wrapper around the RPC command is called `setActivity()` yet isn't really related to setting the state for the kind of Activity that *you're* building.

We know, it's confusing ¯\\_(⊙︿⊙)_/¯ — the naming was logical at the time because it was really about the user's activity in a 3rd party game or service, but now it sorta just feels like activity-ception. Understanding the nuances here aren't super important, and it's why we have guides like this one. But as they say...the more you (at least sort of) know.
</Collapsible>

### rpc.activities.write Scope

To display custom Rich Presence data for a user, your app will need to be authorized with the [`rpc.activities.write` scope](#DOCS_TOPICS_OAUTH2/shared-resources-oauth2-scopes) for that user when you call the **[`authorize()`](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/authorize)** command.
...
When calling [`setActivity()`](#DOCS_DEVELOPER_TOOLS_EMBEDDED_APP_SDK/setactivity), you are expected to pass an `activity` field which is a partial [Activity object](#DOCS_TOPICS_GATEWAY_EVENTS/activity-object-activity-structure) since some fields (like the ones related to spectating) aren't relevant to Activities.

Below is a table with most of the available fields for the activity partial.
...
Now let's take a look at more of a real example. Take a look at the Rich Presence data below that is for a fake Activity:

![Example of a fake game's Rich Presence data](activities-presence-example.png)

To create this display, here is what the `setActivity()` code would look like:
...
<Collapsible title="Example setting presence with an external asset" icon="code">
...
await discordSdk.commands.setActivity({
  activity: { 
    type: 2,
    state: 'Broken Code and Hearts (club edit)',
    details: 'DJ Wump',
    assets: {

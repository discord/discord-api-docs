...
| Third row below title                  | startTimestamp | Converted to a format such as `mm:ss` format such as `12:01`                |
| Third row below title                  | state          | If provided with a `partySize` and `partyMax`                               |
| Third row below title                  | partySize      | In parenthesis next to the `state`, first number in the format `(1 of 4)`   |
| Third row below title                  | partyMax       | In parenthesis next to the `state`, second number in the format `(1 of 4)`  |

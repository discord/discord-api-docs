...
Şimdiyse uygulamanın [bot kullanıcısını](/topics/oauth2#bot-vs-user-accounts) yapılandırarak onun da tıpkı diğer sunucu üyeleri gibi görünmesini ve hareket etmesini sağlayacağız.

Sol taraftaki kenar çubuğunda **Bot**'a tıkla. Bu sayfada botun [ayrıcalıklı amaçlarını](/topics/gateway#privileged-intents) veya diğer kullanıcılar tarafından yüklenip yüklenemeyeceği gibi ayarları yapılandırabilirsin.

<Collapsible title="Amaç nedir?" description="Standart ve ayrıcalıklı amaçlara giriş" icon="question">
Amaçlar, bir [Ağ Geçidi API bağlantısı](/topics/gateway) oluştururken Discord'un uygulamana hangi olayları göndereceğini belirler. Örneğin, kullanıcılar bir mesaja tepki eklediğinde uygulamanın bir şey yapmasını istiyorsan `GUILD_MESSAGE_REACTIONS` (`1 << 10`) amacını gönderebilirsin.

Bazı amaçlar [privileged'tir](/topics/gateway#privileged-intents), yani uygulamanın hassas olarak kabul edilebilecek verilere (mesajların içeriği gibi) erişmesine izin verir. Privileged intent'ler uygulamanın ayarlarındaki **Bot** sayfasında görünür ve açılıp kapatılabilir. Standart, privileged intent dışındakiler herhangi bir ek izin veya yapılandırma gerektirmez.

Amaçlar hakkında daha fazla bilgi ve mevcut amaçların tam listesi, ilişkili olaylarıyla birlikte [Ağ Geçidi dokümantasyonunda](/topics/gateway#gateway-intents) bulunmaktadır.
</Collapsible>
...
- [OAuth2 Kapsamları](/topics/oauth2#oauth2-scopes), uygulamanın yükleme yapan veya yetki veren bir kullanıcı adına hangi verilere erişebileceğini ve eylemleri gerçekleştirebileceğini belirler.
- [İzinler](/topics/permissions#permission-overwrites), Discord'daki diğer kullanıcıların sahip olduğu gibi bot kullanıcıya verilmiş detaylı izinlerdir. Bunlar, yükleyen kullanıcı tarafından onaylanabilir veya daha sonra sunucu ayarlarında veya [izinlerin üzerine yazarak](/topics/permissions#permission-overwrites) güncellenebilir.
</Collapsible>
...
- `applications.commands`, uygulamanın [komutlar](/interactions/application-commands) oluşturmasını sağlar.
...
Dokümantasyonda tüm [OAuth2 kapsamlarının](/topics/oauth2#oauth2-scopes) bir listesine bakabilir veya [izinler](/topics/permissions) hakkında daha fazla bilgi edinebilirsin.
...
Proje, `commands.js` dosyasının altında tanımlanan `ALL_COMMANDS` içerisindeki komutları yüklemek için kullanabileceğin bir `register` komut dosyası içerir. HTTP API'sinin [`PUT /applications/<APP_ID>/commands`](/interactions/application-commands#bulk-overwrite-global-application-commands) uç noktasını çağırarak komutları global komutlar olarak yükler.

Komutlarını özelleştirmek veya ilave komutlar eklemek istersen [komutlar dokümantasyonundaki](/interactions/application-commands#application-command-structure) komut yapısına başvurabilirsin.
...
- **[HTTP API](/reference#http-api)**, Discord'da veri gönderme, veri güncelleme veya bir kaynak hakkında veri alma gibi genel işlemler için kullanılan REST benzeri bir API'dir.
- **[Ağ Geçidi API](/reference#gateway-websocket-api)**, durumu korumak veya Discord sunucusunda gerçekleşen olayları dinlemek için yararlı, WebSocket tabanlı bir API'dir. Bu rehberde bunu kullanmayacağız ancak bir Ağ Geçidi bağlantısının nasıl oluşturulacağı hakkında ve dinleyebileceğin farklı olaylar hakkında daha fazla bilgi [Ağ Geçidi dokümantasyonunda](/topics/gateway) bulunmaktadır.
</Collapsible>
...
Uygulamanı etkileşim almaya hazırlama hakkında daha fazla bilgiyi [etkileşimler dokümantasyonunda](/interactions/receiving-and-responding#receiving-an-interaction) bulabilirsin.
...
Yukarıdaki kod, gelen etkileşime meydana geldiği kanaldaki bir mesajla yanıt veriyor. Modal ile yanıt vermek gibi mevcut tüm yanıt türlerini [dokümantasyonda](/interactions/receiving-and-responding#interaction-callback-type) bulabilirsin.
...
Komut seçenekleri ve bunların yapısı hakkında daha fazla bilgiyi [dokümantasyonlardan](/interactions/application-commands#application-command-option-structure) edinebilirsin.
...
[Mesaj bileşenleriyle](/interactions/message-components#what-is-a-component) bir mesaj gönderirken, tekil payload'lar bir `components` dizisine eklenir. Eyleme geçirilebilir bileşenlerin (butonlar gibi) kod örneğinde görebileceğin bir [eylem satırının](/interactions/message-components#action-rows) içinde olması gereklidir.
...
Kullanıcılar bir mesaj bileşeniyle etkileşime girdiğinde, Discord [etkileşim türü](/interactions/receiving-and-responding#interaction-type) `3` (veya `discord-interactions` kullanılırken `MESSAGE_COMPONENT`) değeri olan bir istek gönderecektir.
...
2. `node-fetch` kullanarak bir webhook çağıran ve istek gövdesinde benzersiz etkileşim `token`'ı ileten [orijinal mesajı siler](/interactions/receiving-and-responding#delete-original-interaction-response). Bu, kanalı temizlemek için yapılır ve böylece diğer kullanıcılar butona tıklayamaz.
3. Oyun için nesne seçeneklerini içeren bir seçim menüsü içeren bir mesaj göndererek isteğe yanıt verir. Payload, `options` dizisi ve [mesajın geçici olduğunu gösteren](/interactions/receiving-and-responding#create-followup-message) `flags: 64` belirteci haricinde önceki payload'a epey benzer görünmelidir.

`options` dizisi, `game.js`'deki `getShuffledOptions()` metodu kullanılarak doldurulur ve bu metot `RPSChoices` değerlerini [mesaj bileşeni seçeneklerinin](/interactions/message-components#select-option-structure) şekline uyacak şekilde değiştirir.
...
Ayrıca, bu kez silinemediği için [takip eden geçici mesajı güncellemek](/interactions/receiving-and-responding#edit-followup-message) için başka bir webhook çağırıyoruz.
...
- API özellikleri hakkında derinlemesine bilgi için **[dokümantasyonları](//)** oku
- Daha küçük, özelliğe has kod örnekleri için bu projedeki `examples/` klasörüne göz at
- Topluluk üyeleri tarafından yürütülen kodlama diline özgü araçlar için **[topluluk kaynaklarına](/topics/community-resources)** göz at
- Discord uygulamalarını [Cloudflare Workers](/tutorials/hosting-on-cloudflare-workers) üzerinde barındırma hakkındaki eğitimimizi oku

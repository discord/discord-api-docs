...
To attempt refreshing, make a `POST` request to the [token URL](/docs/topics/oauth2#shared-resources-oauth2-urls) with the following parameters:
...
Boom; fresh [access token response](/docs/topics/oauth2#authorization-code-grant-access-token-response) with a new `access_token` and `refresh_token`!!

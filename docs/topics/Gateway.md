...
>warn
> Clients are limited to 1000 `IDENTIFY` calls to the websocket in a 24-hour period. This limit is global and across all shards, but does not include `RESUME` calls. Any client that hits that limit will automatically have their bot token reset, and the operator must update their application accordingly. This is to prevent bots from getting into infinite restart loops. When the limit is reached, the owner will receive an email stating that their bot token has been reset, and all active sessions for that bot will be terminated.

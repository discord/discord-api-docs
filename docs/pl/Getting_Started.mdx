...
- [Zakresy OAuth2](/docs/topics/OAuth2#oauth2-scopes) określają, do których danych aplikacja może uzyskiwać dostęp i jakie czynności może wykonywać w imieniu instalującego lub uwierzytelniającego użytkownika.
...
Zobacz listę wszystkich [zakresów OAuth2](/docs/topics/OAuth2#oauth2-scopes) lub przeczytaj więcej na temat [uprawnień](/docs/topics/Permissions) w dokumentacji.
...
Jeśli chcesz spersonalizować polecenia lub dodać ich więcej, możesz odnieść się do struktury poleceń przedstawionej w [ich dokumentacji](/docs/interactions/Application_Commands#application-command-structure).
...
Powyższy kod reaguje na interakcję z wiadomością na kanale, z którego pochodzi. Możesz zobaczyć wszystkie dostępne typy reakcji, takie jak reagowanie oknem, [w dokumentacji](/docs/interactions/Receiving_and_Responding#interaction-callback-type).
...
Więcej o opcjach poleceń i ich strukturze przeczytasz [w dokumentacji](/docs/interactions/Application_Commands#application-command-option-structure).
...
Gdy użytkownicy wejdą w interakcję z elementem wiadomości, Discord wyśle żądanie z [typem interakcji](/docs/interactions/Receiving_and_Responding#interaction-type) `3` (lub wartością `MESSAGE_COMPONENT` przy używaniu `discord-interactions`).
...
Tabela `options` jest wypełniana przy pomocy metody `getShuffledOptions()` w pliku `game.js`, która dostosowuje wartości `RPSChoices` do formatu [opcji elementu wiadomości](/docs/interactions/Message_Components#select-option-structure).

...
> preview
> The user installation context for apps is currently in a public preview and is subject to change. Read details and limitations about the public preview in the change log (TODO: WILL ADD LINK :))

> danger
> Note: right now, I've focused on ability to get through the tutorial. A lot of the details, like the repository name and various TODOs, will be different when the guide is finished.
...
While the tutorial will focus on supporting different [installation contexts](#DOCS_RESOURCES_APPLICATION/installation-context), we'll be building a basic game integration along the way with a wiki lookup with user-specific bookmarking and a server leaderboard.

<Collapsible title="What we'll be building" icon="view">

We'll be building an app that integrates with a fake game. It has four commands (`/link`, `/profile`, `/leaderboard`, and `/wiki`) that can be run in different contexts. TODO: make this more explicit

TODO: GIF of app in action

</Collapsible>

<Collapsible title="Resources used in this guide" description="Overview of the tools and technologies we'll use" icon="list">

- **[GitHub repository](https://github.com/shaydewael/example-game-integration)** where the code from this guide lives.
...
Before we dig in, project code from the [sample app repository](https://github.com/shaydewael/example-game-integration).
...
> info
> We'll be developing our app locally with a little help from [ngrok](https://ngrok.com/), but the code is also available on [Glitch 🎏](#TODO) if you prefer (our [Getting Started](#DOCS_QUICK_START_GETTING_STARTED) has instructions for using Glitch).
...
```shell
# ssh
git clone git@github.com:shaydewael/example-game-integration.git
```
...
**TODO: i'm going to refactor the app to not used a privileged intent, but this is here for now to make sure it's runnable**

The sample app fetches members in the server when constructing a fake game leaderboard. Getting server members requires a special permission called a [privileged intent](#DOCS_TOPICS_GATEWAY/privileged-intents), so we'll add that to our app.
...
An app's **[installation context](#DOCS_RESOURCES_APPLICATION/installation-context)** defines how it's installed: to a server, to a user, or both.

We're going to configure our app to support both installation contexts since we want it available to install to servers in addition to users. [**TODO: maybe a bit of the "why" here.**]
...
The default install settings of your app determines the default [scopes](#DOCS_TOPICS_OAUTH2/shared-resources-oauth2-scopes) and [bot user permissions](#DOCS_TOPICS_PERMISSIONS) for each supported installation context. At the moment, apps installed to a user context only support the `applications.commands` scope which allows your app to install [commands](#DOCS_INTERACTIONS_APPLICATION_COMMANDS).

###### Update Install Link

Before adding default install settings, we need to enable Discord provided links. We won't be covering custom URLs in this tutorial, but you can read more about customizing the installation flow in the [OAuth2](#DOCS_TOPICS_OAUTH2/oauth2) documentation.

Under the **Install Link** section, select `Discord Provided Link` from the dropdown. Once its selected, the **Default Install Settings** will appear.

###### Adding Default Install Settings

Under the new **Default Install Settings** section, set the following:
...
![Installation settings in App Settings](user-install-settings.png)
...
With our Interactions Endpoint URL set up, we should now be able to run our app's commands. Go to your app's DM and run `/profile`, and your app should respond with a sample game profile.
...
To see which command was run, you can look at the `data` object, TODO LINK TO DOCS.
...
`context`) tells you which [interaction context](#TODO) the command was invoked from. I triggered the command from my app's DM, so `context` is `1` (or `BOT_DM`).

With interaction context, something to keep in mind in `BOT_DM` is only the *DM with your bot user*. If you run the same command in a DM with your bestie (or enemy), or in a group DM, the interaction context will be `PRIVATE_CHANNEL` (`2`).
...
Understanding the authorization owner can be helpful when handling interactions from message components for apps installed to a user, which is discussed more in the [message component interactions](#DOCS_TUTORIALS_DEVELOPING_A_USER_INSTALLABLE_APP/using-metadata-for-message-component-interactions) section. Or you can find technical details in the [Authorizing Integration Owners](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/authorizing-integration-owners-object) documentation.
...
All of these values can be helpful when deciding how you want your app to [respond to the interaction](#DOCS_INTERACTIONS_RECEIVING_AND_RESPONDING/responding-to-an-interaction). For example, maybe you want your app to respond ephemerally when a specific command is invoked from a server, which the sample app does for the `/profile` command.
...
![Sample game command response](user-install-game-message.png)
...
There are two fields that can be helpful in this scenario (and more complicated ones)—

###### `interaction_metadata`

TODO: interaction vs interaction_metadata
Messages created in response to an interaction will include an [`interaction_metadata` object](#TODO) that includes information about 

###### `authorizing_integration_owners`

`authorizing_integration_owners` was touched on above, but it's worth highlighting again since it's especially helpful when handling message component interactions for apps installed to users. TODO
...
Hopefully at this point, you have an app that supports both installation contexts and understand the basics of using metadata to support different contexts. Now you can go explore the documentation for details, or play with the sample app to develop more complex features.

<Container>
  <Card title="Interactions documentation" link="#todo">
    Explore the Interactions documentation to learn more about receiving and responding to commands and message components
  </Card>
  <Card title="Github repository" link="#open-invite-dialog">
    TODO
  </Card>
</Container>

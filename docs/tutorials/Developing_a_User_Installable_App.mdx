...
To set up a public endpoint we'll start our app, which runs an [Express](#TODO) server, then use [ngrok](#TODO) to expose our server publicly. 
...
Go to your [app's settings](#TODO) and on the **General Information** page under **Interaction Endpoint URL**, paste your new ngrok URL and append `/interactions` (it'll be something like `https://84c5df474.ngrok-free.dev/interactions`).
...
[`context`](#TODO) tells you which [interaction context](#TODO) the command was invoked from. I triggered the command from my app's DM, so `context` is `1` (or `BOT_DM`).
...
[`authorizing_integration_owners`](#TODO) provides data about any ID relevant to the [installation context(s)](#TODO) associated with the interaction.
...
###### `app_permissions`
[`app_permissions`](#TODO) are the bitwise set of permissions your app has in the place where the interaction was triggered. The permissions your app has will be different for DMs with your app, in servers, and G(DM)s with other users.

In the sample payload, the value is `'442368'`, which means my app has [TODO: add permissions].

All of these values can be helpful when deciding how you want your app to [respond to the interaction](#TODO). For example, maybe you want your app to respond ephemerally when a specific command is invoked from a server, which the sample app does for the `/profile` command.

### Using metadata to respond based on context

As mentioned above, the `/profile` command for our app will respond ephemerally, meaning only the invoking user will see the response, when invoked from a server. If it's invoked within a DM with the bot user, it'll respond with a non-ephemeral message. In the project, you can see this logic in `app.js` on [TODO: line #]:

<Collapsible title="Sample interaction payload" icon="code">
TODO: code sample for this
</Collapsible>

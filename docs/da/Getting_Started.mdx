...
- [OAuth2-anvendelsesområder](/docs/topics/OAuth2#oauth2-scopes) bestemmer, hvilken dataadgang og hvilke handlinger din app har adgang til, tildelt på vegne af en installerende eller bekræftende bruger.
...
Se en liste med alle [OAuth2-anvendelsesområder](/docs/topics/OAuth2#oauth2-scopes), eller læs mere om [tilladelser](/docs/topics/Permissions) i dokumentationen.
...
Hvis du ønsker at tilpasse dine kommandoer eller tilføje ekstra kommandoer, kan du refererer kommandostrukturen i [kommandodokumentationen](/docs/interactions/Application_Commands#application-command-structure).
...
Koden herover reagerer på interaktionen med en besked i kanalen, hvor kommandoen kom fra. Du kan se alle tilgængelige svartyper såsom besvarelse med en modal [i dokumentationen](/docs/interactions/Receiving_and_Responding#interaction-callback-type).
...
Du kan læse mere om kommando-valgmuligheder og deres struktur [i dokumentationen](/docs/interactions/Application_Commands#application-command-option-structure).
...
Når brugere interagerer med en beskedkomponent, sender Discord en anmodning med [interaktionstypen](/docs/interactions/Receiving_and_Responding#interaction-type) `3` (eller `MESSAGE_COMPONENT`-værdien, når der anvendes `discord-interactions`).
...
`options`-sættet befolkes ved hjælp af `getShuffledOptions()`-metoden i `game.js`, der manipulerer `RPSChoices`-værdierne, så de passer til formen på [beskedkomponentvalgene](/docs/interactions/Message_Components#select-option-structure).

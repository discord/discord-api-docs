...
- [OAuth2-scopes](/docs/topics/OAuth2#shared-resources-oauth2-scopes) bepalen de datatoegang en acties waarvan je app gebruik mag maken, toegekend uit naam van een gebruiker die de app installeert of authentiseert.
...
Bekijk een lijst met alle [OAuth2-scopes](/docs/topics/OAuth2#shared-resources-oauth2-scopes) of lees meer over [machtigingen](/docs/topics/Permissions) in de documentatie.
...
Als je je opdrachten wilt aanpassen of extra opdrachten wilt toevoegen, kun je verwijzen naar de opdrachtstructuur in de [opdrachtdocumentatie](/docs/interactions/Application_Commands#application-command-object-application-command-structure).
...
Je kunt meer lezen over opdrachtopties en hun structuur [in de documentatie](/docs/interactions/Application_Commands#application-command-object-application-command-option-structure).
...
Als gebruikers interactie aangaan met een berichtonderdeel, verstuurt Discord een verzoek met één [interactietype](/docs/interactions/Receiving_and_Responding#interaction-object-interaction-type) uit `3` opties (of de waarde van het `MESSAGE_COMPONENT` als `discord-interactions` wordt gebruikt).
...
De `options`-lijst wordt gevuld met behulp van de `getShuffledOptions()`-methode in `game.js`, die de `RPSChoices`-waarden manipuleert om te passen bij de vorm van de [berichtcomponentopties](/docs/interactions/Message_Components#select-menu-object-select-option-structure).

...
Attachments are uploaded to the CDN and use the `media.discordapp.net` domain (though accessible via the cdn subdomain, it restricts the width/height paramaters for images)

The attachments endoint: 

/attachments/[channel_id](#DOCS_RESOURCES_CHANNEL/channel-object)/[attachment_id](#DOCS_RESOURCES_CHANNEL/attachment-object)/[file_name](#DOCS_RESOURCES_CHANNEL/attachment-object)

*Note* `file_name` is inclusive of the extension, if you're uploading foo.png for example, the entire `file_name` in the above example would be "foo.png"

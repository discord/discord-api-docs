...
All [Auto Moderation](/docs/resorces/auto-moderation) related events are only sent to bot users which have the `MANAGE_GUILD` permission.
...
\* `message_id` will not exist if message was blocked by [Auto Moderation](/docs/resorces/auto-moderation) or content was not part of any message

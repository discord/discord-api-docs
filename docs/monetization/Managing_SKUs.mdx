...
The premium items and subscriptions you offer in your app are represented by SKUs. **SKU** stands for Stock Keeping Unit, and it's a unique identifier for each of your premium offerings.
...
- SKU prices must be be selected from the list of available prices.
...
Your list of benefits displayed on the Premium App Store view, the App Directory, and during the purchase and cancellation flows to explain to users the benefits of your premium offering. These benefits can have:
- Up to 6 benefits
...
When setting the price for your SKU, 
...
When you first create a SKU, it will be in an `Unavailable` state. This means that the SKU is not yet available for purchase by users. You can edit the SKU to add a price, benefits, and other details before publishing it.
...
You can only make API calls or use th Embedded App SDK to grant entitlements for this SKU.

### Unpublishing a SKU

This action unpublishes a SKU from the Store and the API. Unpublished SKUs will not be available for purchase by users.

> warn
> **User Impact of Unpublishing a SKU**
...
- [Implementing App Subscriptions](#DOCS_MONETIZATION_IMPLEMENTING_APP_SUBSCRIPTIONS)
- [Implementing One-Time Purchases](#DOCS_MONETIZATION_IMPLEMENTING_ONE-TIME_PURCHASES) 
- Implementing IAP (In-App Purchases) in Activities (✨ coming soon ✨)

--- 

## Promoting your Premium Offerings

Once you've created and published your SKUs, you can promote them to your users. Here are a few ways to promote your premium offerings:

- Users can view your subscriptions and items that have been published to your Store
- Link directly to a specific SKU
- Link directly to your Store
- Include a button with a [premium button style](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS/button-object-button-styles) in message components
- When building an activity, you can use the Embedded App SDK to offer in-activity purchases

### User Access to Your Store
...
> info
> Only subscriptions and items that have been published to the Store will be visible to users in the Store view.

#### Accessing your Store view from a Bot User's Profile
![Accessing the store as a user](botuser-profile.png)

#### Subscriptions in Your Store View
![Subscriptions in your Store View](premium-subscriptions.png)

#### items in Your Store View
...
### Including a Premium Button in Message Components

You can prompt users to purchase item or subscription SKUs using a [button](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS/buttons) with a [premium style](#DOCS_INTERACTIONS_MESSAGE_COMPONENTS/button-object-button-styles) and a `sku_id`. You can use this premium button style anywhere you would you normally use message components, such as in a command response.

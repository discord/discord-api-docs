...
You can prompt users to purchase item or subscription SKUs using a [button](/docs/interactions/message-components#buttons) with a [premium style](/docs/interactions/message-components#button-object-button-styles) and a `sku_id`. You can use this premium button style anywhere you would use message components, such as in a command response.

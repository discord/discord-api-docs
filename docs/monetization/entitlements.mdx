...
| type           | integer           | [Type of entitlement](/monetization/entitlements#entitlement-types)                         |
...
<RoutePill method="GET">/applications/[\{application.id\}](/resources/application#application-object)/entitlements</RoutePill>
...
| exclude_ended? | [boolean](/reference#boolean-query-strings) | Whether or not ended entitlements should be omitted  |
...
<RoutePill method="POST">/applications/[\{application.id\}](/resources/application#application-object)/entitlements/[\{entitlement.id\}](/monetization/entitlements#entitlement-object)/consume</RoutePill>

For One-Time Purchase consumable SKUs, marks a given entitlement for the user as consumed. The entitlement will have `consumed: true` when using [List Entitlements](/monetization/entitlements#list-entitlements).

Returns a `204 No Content` on success.

## Create Test Entitlement

<RoutePill method="POST">/applications/[\{application.id\}](/resources/application#application-object)/entitlements</RoutePill>
...
<RoutePill method="DELETE">/applications/[\{application.id\}](/resources/application#application-object)/entitlements/[\{entitlement.id\}](/monetization/entitlements#entitlement-object)</RoutePill>
...
If your app has monetization enabled, it will have access to a new [`PREMIUM_REQUIRED` interaction response (`type: 10`)](/interactions/receiving-and-responding#interaction-callback-type). This can be sent in response to all interaction types except for `APPLICATION_COMMAND_AUTOCOMPLETE` and `PING`. The response does not allow returning the `content`, `embeds`, or `attachments` fields.
...
To check what the current guild or user has entitlements to, your app can inspect the `entitlements` field. `entitlements` is an array of [entitlement objects](/monetization/entitlements#entitlement-object) for the current guild and user.

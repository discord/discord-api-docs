...
Als Nächstes konfigurieren wir den [Botbenutzer](/topics/oauth2#bot-vs-user-accounts) für deine App, damit er wie andere Servermitglieder aussieht und handelt.

Klick in der linken Seitenleiste auf **Bot**. Auf dieser Seite kannst du Einstellungen wie seine [Privileged Intents](/topics/gateway#privileged-intents) festlegen, oder ob er von anderen Benutzern installiert werden kann.

<Collapsible title="Was sind Intents?" description="Einführung in Standard Intents und Privileged Intents" icon="question">
Mit Intents wird festgelegt, welche Events Discord an deine App sendet, wenn du eine [Gateway-API-Verbindung](/topics/gateway) erstellst. Wenn deine App zum Beispiel etwas tun soll, wenn Benutzer einer Nachricht eine Reaktion hinzufügen, kannst du den Intent `GUILD_MESSAGE_REACTIONS` (`1 << 10`) vergeben.

Manche Intents sind [Privileged](/topics/gateway#privileged-intents), weil sie deiner App Zugriff auf Daten gewähren, die vertraulich sein können (wie der Inhalt von Nachrichten). Privileged Intents werden auf der **Bot**-Seite deiner App-Einstellungen angezeigt und können dort aktiviert oder deaktiviert werden. Alle anderen Intents, die nicht dazu zählen, werden Standard Intents genannt und benötigen keine zusätzlichen Berechtigungen oder Konfigurationen.

Mehr Informationen zu Intents und eine Liste aller verfügbaren Intents mit ihren zugehörigen Events findest du in der [Dokumentation zu Gateways](/topics/gateway#gateway-intents).
</Collapsible>
...
- [OAuth2 Scopes](/topics/oauth2#oauth2-scopes) legen den Datenzugriff und die möglichen Aktionen deiner App fest. Sie werden so von installierenden oder authentifizierenden Benutzern übernommen.
- [Permissions](/topics/permissions#permission-overwrites) sind die granularen Berechtigungen für deinen Botbenutzer, wie sie auch andere Discord-Benutzer haben. Sie können vom installierenden Benutzer gewährt oder später in den Servereinstellungen oder mit [Permission Overwrites](/topics/permissions#permission-overwrites) aktualisiert werden.
</Collapsible>
...
- `applications.commands` ermöglicht es deiner App, [Commands](/interactions/application-commands) (Befehle) zu erstellen.
...
Es gibt eine Liste der [Oauth2 Scopes](/topics/oauth2#oauth2-scopes) und du kannst in der Dokumentation mehr zu [Permissions](/topics/permissions) lesen.
...
Das Projekt enthält ein `register`-Skript, das du benutzen kannst, um die Befehle in `ALL_COMMANDS` (am Ende von `commands.js`) zu installieren. Sie werden als globale Befehle installiert, indem der Endpunkt [`PUT /applications/<APP_ID>/commands`](/interactions/application-commands#bulk-overwrite-global-application-commands) der HTTP-API angefragt wird.

Wenn du deine Befehle anpassen oder welche hinzufügen willst, kannst du den Abschnitt Command Structure in der [Dokumentation zu Befehlen](/interactions/application-commands#application-command-structure) zu Rate ziehen.
...
- **[HTTP API](/reference#http-api)** ist eine REST-API für allgemeine Operationen wie das Senden und Aktualisieren von Daten in Discord oder die Abfrage von Daten zu einer Ressource.
- **[Gateway API](/reference#gateway-websocket-api)** ist eine WebSocket-basierte API, mit deren Hilfe der Zustand beibehalten oder Events eines Discord-Servers „gehört“ werden können. Wir werden sie nicht in diesem Guide benutzen. Mehr Informationen zur Erstellung einer Gateway-Verbindung und zu den verschiedenen Events, die gehört werden können, findest du in der [Dokumentation zu Gateway](/topics/gateway).
</Collapsible>
...
In der [Dokumentation zu Interactions](/interactions/receiving-and-responding#receiving-an-interaction) kannst du mehr darüber erfahren, wie du deine App auf eingehende Interaktionen vorbereiten kannst.
...
Der Code oben reagiert auf die Interaktion mit einer Nachricht in dem Kanal, wo sie ausgelöst wurde. [In der Dokumentation](/interactions/receiving-and-responding#interaction-callback-type) findest du alle verfügbaren Antworttypen, zum Beispiel mit einem Modal.
...
[In der Dokumentation](/interactions/application-commands#application-command-option-structure) kannst du mehr über Befehlsoptionen und ihre Struktur lesen.
...
Wenn eine Nachricht mit [Message Components](/interactions/message-components#what-is-a-component) (Nachrichtenkomponenten) gesendet wird, werden die individuellen Payloads an ein `components`-Array angehängt. Ausführbare Komponenten (wie Buttons) müssen wie im Beispielcode in einer [Action Row](/interactions/message-components#action-rows) sein.
...
Wenn Benutzer mit einer Message Component interagieren, sendet Discord eine Anfrage mit dem [Interaction Type](/interactions/receiving-and-responding#interaction-type) `3` (oder dem Wert von `MESSAGE_COMPONENT`, wenn `discord-interactions` verwendet wird).
...
Wir rufen außerdem einen weiteren WebHook auf, der dieses Mal [die ephemerale Nachricht aktualisiert](/interactions/receiving-and-responding#edit-followup-message), da sie nicht gelöscht werden kann.
...
- Lies **[die Dokumentation](//)** für detaillierte Informationen zu API-Features.
- Schau dir im `examples/`-Ordner dieses Projekts weitere kleinere Code-Beispiele für spezifische Features an.
- Entdecke die **[Community-Ressourcen](/topics/community-resources)** für sprachspezifische Tools von Community-Mitgliedern.
- Lies unser Tutorial zum [Hosten von Discord-Apps auf Cloudflare Workers](/tutorials/hosting-on-cloudflare-workers).

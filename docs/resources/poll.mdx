...
|-------------------|-----------------------------------------------------------------------------------------------------|-----------------------------------------------------------------|
| question          | [Poll Media Object](/resources/poll#poll-media-object-structure)             | The question of the poll. Only `text` is supported.             |
| answers           | List of [Poll Answer Objects](/resources/poll#poll-answer-object-structure) | Each of the answers available in the poll.                      |
| expiry            | ?IS08601 timestamp                                                                                  | The time when the poll ends.                                    |
| allow_multiselect | boolean                                                                                             | Whether a user can select multiple answers                      |
| layout_type       | integer                                                                                             | The [layout type](/resources/poll#layout-type) of the poll |
| results?          | [Poll Results Object](/resources/poll#poll-results-object-structure)       | The results of the poll                                         |
...
It is similar but not exactly identical to the main [poll object](/resources/poll#poll-object-structure).
...
|-------------------|-----------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| question          | [Poll Media Object](/resources/poll#poll-media-object-structure)             | The question of the poll. Only `text` is supported.                                      |
| answers           | List of [Poll Answer Objects](/resources/poll#poll-answer-object-structure) | Each of the answers available in the poll, up to 10                                      |
| duration          | integer                                                                                             | Number of hours the poll should be open for, up to 7 days                                |
| allow_multiselect | boolean                                                                                             | Whether a user can select multiple answers                                               |
| layout_type?      | integer                                                                                             | The [layout type](/resources/poll#layout-type) of the poll. Defaults to... DEFAULT! |
...
| poll_media  | [Poll Media Object](/resources/poll#poll-media-object-structure) | The data of the answer |
...
| answer_counts | List of [Poll Answer Count Object](/resources/poll#poll-answer-count-object-structure) | The counts for each answer                    |
...
For creating a poll, see [Create Message](/resources/channel#create-message). After creation, the poll message cannot be edited.

Apps are not allowed to vote on polls. No rights! :)

## Get Answer Voters

<RoutePill method="GET">/channels/[\{channel.id\}](/resources/channel#channel-object)/polls/[\{message.id\}](/resources/channel#message-object)/answers/[\{answer_id\}](/resources/poll#poll-answer-object)</RoutePill>
...
| Field | Type                                              | Description                     |
|-------|---------------------------------------------------|---------------------------------|
| users | array of [user](/resources/user#user-object) | Users who voted for this answer |

## End Poll

<RoutePill method="POST">/channels/[\{channel.id\}](/resources/channel#channel-object)/polls/[\{message.id\}](/resources/channel#message-object)/expire</RoutePill>

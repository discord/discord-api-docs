...
| locale?                 | string                                                                          | the user's chosen [language option](/reference#locales)                                 | identify              |
| verified?               | boolean                                                                         | whether the email on this account has been verified                                     | email                 |
| email?                  | ?string                                                                         | the user's email                                                                        | email                 |
| flags?                  | integer                                                                         | the [flags](/resources/user#user-flags) on a user's account                             | identify              |
| premium_type?           | integer                                                                         | the [type of Nitro subscription](/resources/user#premium-types) on a user's account     | identify              |
| public_flags?           | integer                                                                         | the public [flags](/resources/user#user-flags) on a user's account                      | identify              |
| avatar_decoration_data? | ?[avatar decoration data](/resources/user#avatar-decoration-data-object) object | data for the user's avatar decoration                                                   | identify              |
...
| `1 << 10` | TEAM_PSEUDO_USER         | User is a [team](/topics/teams)                                                                                                           |
...
| `1 << 19` | BOT_HTTP_INTERACTIONS    | Bot uses only [HTTP interactions](/interactions/receiving-and-responding#receiving-an-interaction) and is shown in the online member list |
...
|--------|-----------|-----------------------------------------------------------|
| asset  | string    | the [avatar decoration hash](/reference#image-formatting) |
...
| type          | string  | the [service](/resources/user#services) of this connection                      |
...
| visibility    | integer | [visibility](/resources/user#visibility-types) of this connection               |
...
| metadata          | object  | object mapping [application role connection metadata](/resources/application-role-connection-metadata#application-role-connection-metadata-object) keys to their `string`-ified value (max 100 characters) for the user on the platform a bot has connected |

## Get Current User

<RoutePill method="GET">/users/@me</RoutePill>
...
<RoutePill method="GET">/users/[\{user.id\}](/resources/user#user-object)</RoutePill>

Returns a [user](/resources/user#user-object) object for a given user ID.

## Modify Current User

<RoutePill method="PATCH">/users/@me</RoutePill>

Modify the requester's user account settings. Returns a [user](/resources/user#user-object) object on success. Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
| banner   | ?[image data](/reference#image-data) | if passed, modifies the user's banner                                            |

## Get Current User Guilds

<RoutePill method="GET">/users/@me/guilds</RoutePill>

Returns a list of partial [guild](/resources/guild#guild-object) objects the current user is a member of. For OAuth2, requires the `guilds` scope.
...
| with_counts | [boolean](/reference#boolean-query-strings) | include approximate member and presence counts in response | false    | false   |

## Get Current User Guild Member

<RoutePill method="GET">/users/@me/guilds/[\{guild.id\}](/resources/guild#guild-object)/member</RoutePill>

Returns a [guild member](/resources/guild#guild-member-object) object for the current user. Requires the `guilds.members.read` OAuth2 scope.

## Leave Guild

<RoutePill method="DELETE">/users/@me/guilds/[\{guild.id\}](/resources/guild#guild-object)</RoutePill>

Leave a guild. Returns a 204 empty response on success. Fires a [Guild Delete](/topics/gateway-events#guild-delete) Gateway event and a [Guild Member Remove](/topics/gateway-events#guild-member-remove) Gateway event.

## Create DM

<RoutePill method="POST">/users/@me/channels</RoutePill>

Create a new DM channel with a user. Returns a [DM channel](/resources/channel#channel-object) object (if one already exists, it will be returned instead).
...
Create a new group DM channel with multiple users. Returns a [DM channel](/resources/channel#channel-object) object. This endpoint was intended to be used with the now-deprecated GameBridge SDK. Fires a [Channel Create](/topics/gateway-events#channel-create) Gateway event.
...
<RoutePill method="GET">/users/@me/connections</RoutePill>
...
<RoutePill method="GET">/users/@me/applications/[\{application.id\}](/resources/application#application-object)/role-connection</RoutePill>

Returns the [application role connection](/resources/user#application-role-connection-object) for the user. Requires an OAuth2 access token with `role_connections.write` scope for the application specified in the path.

## Update Current User Application Role Connection

<RoutePill method="PUT">/users/@me/applications/[\{application.id\}](/resources/application#application-object)/role-connection</RoutePill>

Updates and returns the [application role connection](/resources/user#application-role-connection-object) for the user. Requires an OAuth2 access token with `role_connections.write` scope for the application specified in the path.

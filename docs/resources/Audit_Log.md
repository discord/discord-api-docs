...
Many audit log events include a `changes` array in their [entry object](#TODO). The structure for the individual changes varies based on the event type and its changed objects, so apps shouldn't depend on a single pattern of handling audit log events.
...
The following table details the possible keys in the `changes` array for the repective objects. The types and descriptions for the keys are in the object's documentation.

| Object Changed                                                                                                                                  | Change Keys               |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| [Channel](#DOCS_RESOURCES_CHANNEL/channel-object)                                                                                               | `application_id`, `bitrate`, `default_auto_archive_duration`, `id`, `name`, `nsfw`, `permission_overwrites`, `position`, `rate_limit_per_user`, `topic`, `type`, `user_limit`                         |
| [Channel Overwrite](#DOCS_RESOURCES_CHANNEL/overwrite-object) | `id`, `type`, `allow`, `deny` |
| [Command Permission](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-permissions-object-application-command-permissions-structure) | snowflake\* |
| [Emoji](#DOCS_RESOURCES_EMOJI/emoji-object)                                                                                                     | `name`                      |
| [Guild](#DOCS_RESOURCES_GUILD/guild-object)                                                                                                     | `afk_channel_id`, `afk_timeout`, `banner_hash`, `default_message_notifications`, `description`, `discovery_splash_hash`, `explicit_content_filter`, `icon_hash`, `id`, `mfa_level`, `name`, `owner_id`, `preferred_locale`, `premium_progress_bar_enabled`, `prune_delete_days`, `public_updates_channel_id`, `region`, `rules_channel_id`, `splash_hash`, `system_channel_flags`, `system_channel_id`, `vanity_url_code`, `verification_level`, `widget_channel_id`, `widget_enabled`                       |
| [Guild Scheduled Event](#DOCS_RESOURCES_GUILD_SCHEDULED_EVENT/guild-scheduled-event-object)                                                     | `channel_id`, `description`, `entity_type`, `image_hash`, `location`, `name`, `privacy_level`, `status`                       |
| [Integration](#DOCS_RESOURCES_GUILD/integration-object) | `enable_emoticons`, `expire_behavior`, `expire_grace_period`, `type` | 
| [Invite](#DOCS_RESOURCES_INVITE/invite-object) and [Invite Metadata](#DOCS_RESOURCES_INVITE/invite-metadata-object)                                                                                                  | `channel_id`, `code`, `inviter_id`, `max_age`, `max_uses`, `temporary`, `uses`                         |
| [Member](#DOCS_RESOURCES_GUILD/guild-member-object)                                                                                             | `avatar_hash`, `communication_disabled_until`, `deaf`, `mute`, `nick`                     |
| [Partial Role](#DOCS_TOPICS_PERMISSIONS/role-object)\*\* | `$add`, `$remove` |
| [Stage Instance](#DOCS_RESOURCES_STAGE_INSTANCE/stage-instance-object)                                                                          | `privacy_level`, `topic`                      |
| [Sticker](#DOCS_RESOURCES_STICKER/sticker-object)                                                                                               | `asset`, `available`, `description`, `format_type`, `guild_id`, `id`, `name`, `tags`                       |
| [Thread](#DOCS_RESOURCES_CHANNEL/thread-metadata-object)                                                                                        | `archived`, `auto_archive_duration`, `invitable`, `locked`, `name`, `rate_limit_per_user`, `type`, `unarchived`                      |
...
\* `APPLICATION_COMMAND_PERMISSIONS_UPDATE` events use a snowflake as the change key. The `changes` array contains `old_value` and `new_value` as the key to different objects, each of which are respectively set to the old and updated permissions object.

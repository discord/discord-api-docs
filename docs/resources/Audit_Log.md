...
The following table details the possible keys in the `changes` array for the repective objects. The change key can be most field names that can change for the object, unless any exceptions are noted. Types and descriptions for the keys can be found in the object's documentation.

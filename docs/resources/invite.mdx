...
| is_nickname_changeable?     | [boolean](/docs/reference#boolean-query-strings)                                                   | the members ability to change their nickname by default, returned from the `GET /invites/<code>` endpoint when `?with_permissions` is `true` |

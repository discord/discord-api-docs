...
| roles?                      | array of partial [role](/docs/topics/permissions#role-object) objects                              | the roles assigned to the user upon accepting the invite. Contains a limited amount of [role](/docs/topics/permissions#role-object) information |
...
```json
{
  ...
  "target_type": 1, // InviteTargetType.STREAM
  "target_user": {
    "id": "165176875973476352",
    "username": "bob",
    "avatar": "deadbeef",
...
Gets the users allowed to see and accept this invite. Response is a CSV file with the header `user_id` and each user ID from the original file passed to [invite create](/docs/resources/channel#create-channel-invite) on its own line. Requires the caller to be the inviter, or have `MANAGE_GUILD` permission, or have `VIEW_AUDIT_LOG` permission.

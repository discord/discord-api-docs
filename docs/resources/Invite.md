...
| guild_scheduled_event?      | [Guild Scheduled Event](#DOCS_RESOURECES_GUILD_SCHEDULED_EVENT/guild-scheduled-event-object) | guild scheduled event data, only included if `guild_scheduled_event_id` contains a valid guild scheduled event id                 |

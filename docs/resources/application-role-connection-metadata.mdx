...
A representation of role connection metadata for an [application](/resources/application).

When a guild has added a bot and that bot has configured its [`role_connections_verification_url`](/resources/application#application-object) (in the developer portal), the application will render as a potential verification method in the guild's role verification configuration.
...
When a user connects their account using the bot's [`role_connections_verification_url`](/resources/application#application-object), the bot will [update a user's role connection with metadata](/resources/user#update-current-user-application-role-connection) using the OAuth2 `role_connections.write` scope.
...
|----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| type                       | [ApplicationRoleConnectionMetadataType](/resources/application-role-connection-metadata#application-role-connection-metadata-type) | type of metadata value                                                                           |
...
<RoutePill method="GET">/applications/[\{application.id\}](/resources/application#application-object)/role-connections/metadata</RoutePill>

Returns a list of [application role connection metadata](/resources/application-role-connection-metadata#application-role-connection-metadata-object) objects for the given application.

## Update Application Role Connection Metadata Records

<RoutePill method="PUT">/applications/[\{application.id\}](/resources/application#application-object)/role-connections/metadata</RoutePill>

Updates and returns a list of [application role connection metadata](/resources/application-role-connection-metadata#application-role-connection-metadata-object) objects for the given application.

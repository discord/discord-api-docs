...
| message_reference?            | [message reference](#DOCS_RESOURCES_CHANNEL/message-object-message-reference-structure) object                                                  | reference data sent with crossposted messages, replies, and system messages about pinned messages and channel follows        |
...
\*\*\*\*\* This field is only returned for messages with `type`s of `6` (CHANNEL_PINNED_MESSAGE) and `19` (REPLY). If the `referenced_message` field is not present, then the backend did not attempt (or failed) to fetch the message that was being referenced, so its state is unknown. If the field exists but is null, the referenced message was deleted.

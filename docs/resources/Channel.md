...
| flags? | integer bit set of [message flags](#DOCS_RESOURCES_CHANNEL/message-object-message-flags) | flags set for the message, sent only if one or more flags are set |
| message\_reference? | [message reference](#DOCS_RESOURCES_CHANNEL/message-object-message-reference) object | sent with crossposted messages |
...
| ---- | ----- | ----------- |
| CROSSPOSTED | `0x00000001` | The message has been crossposted (published) to other channels |
| IS\_CROSSPOST | `0x00000002` | The message is a crosspost of a message from another channel |
| SUPPRESS\_EMBEDS | `0x00000004` | Do not include any embeds with this message |

###### Message Reference

| Field | Type | Description |
| ----- | ---- | ----------- |
| message\_id | snowflake | id of the referenced message |
| channel\_id | snowflake | id of the channel where the referenced message was originally sent |
| guild\_id? | snowflake | id of the guild where the referenced message was originally sent |
...
| rate\_limit\_per\_user | integer | amount of seconds a user has to wait before sending another message (0-21600); bots, as well as users with the permission `manage_messages` or `manage_channel`, are unaffected | Text |

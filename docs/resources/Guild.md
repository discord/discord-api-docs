...
| approximate_member_count?     | integer                                                                             | approximate number of members in this guild, returned from the `GET /guild/<id>` and `GET /users/@me/guilds` endpoints when `with_counts` is `true`                    |
| approximate_presence_count?   | integer                                                                             | approximate number of non-offline members in this guild, returned from the `GET /guild/<id>` and `GET /users/@me/guilds` endpoints when `with_counts` is `true`        |

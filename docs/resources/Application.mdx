...
| team                               | ?[team](/docs/topics/Teams#team-object) object                                                                            | If the app belongs to a team, this will be a list of the members of that team                                                                                                                                                                                                                           |
...
| flags?                             | integer                                                                                                                               | App's public [flags](/docs/resources/Application#application-flags)                                                                                                                                                                                                                  |
...
| integration_types_config?          | dictionary with keys of [application integration types](/docs/resources/Application#application-integration-types) | [In preview](/docs/Change_Log#user-installable-apps-preview). Default scopes and permissions for each supported installation context. Value for each key is an [integration type configuration object](/docs/resources/Application#application-integration-type-configuration-object) |
...
| Field       | Type             | Description                                                                                            |
|-------------|------------------|--------------------------------------------------------------------------------------------------------|
| scopes      | array of strings | [Scopes](/docs/topics/OAuth2#oauth2-scopes) to add the application to the server with |
...
Apps installed in a server context (server-installed apps) must be authorized by a server member with the [`MANAGE_GUILD`](/docs/topics/Permissions#bitwise-permission-flags) permission. Server-installed apps are *visible* to all members of the server, but other factors (like [command permissions](/docs/interactions/Application_Commands#permissions)) determine where and when specific members can interact with the app.

During installation, server-installed apps are authorized with a specific set of [OAuth2 scopes](/docs/topics/OAuth2#oauth2-scopes) and [bot user permissions](/docs/topics/Permissions) that determine what resources and data the app can access in that server.
...
Apps that support the user installation context are visible across all of an authorizing user's servers, DMs, and GDMs, but are forced to respect the user's permissions in the surface where the app is being used. For example, if a user invokes a command for a user-installed app from a server's channel where they don't have permission to send messages, the app won't be able to [respond to an interaction](/docs/interactions/Receiving_and_Responding#interaction-callback-type) with a non-ephemeral message. Details about how the installation context of a command affects interactions is in the [interaction context](/docs/interactions/Application_Commands#interaction-contexts) documentation.

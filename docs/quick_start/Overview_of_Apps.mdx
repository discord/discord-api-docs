...
2. Apps installed **to a user account**. Apps installed to a user is visible *only* to that user, across all of their servers and (G)DMs. Apps installed to a user have [limited functionality](#TODO)—for example, they cannot take actions on behalf of the installing user (like updating settings, sending messages, or creating new channels).

The installation context of an app can be limited by the developer when [configuring the app](#TODO), and by the user authorizing the app during the installation flow. Details about installation contexts are in the [application resource documentation](#DOCS_RESOURCES_APPLICATION/installation-context).

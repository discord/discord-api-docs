...
This PR also documents the `application` field on the `READY` gateway event, which is a partial [application object](#DOCS_TOPICS_APPLICATION/application-object) containing `id` and `flags`.

...
- **Command-level permissions** are set up by an admin for a specific *command* in their server. These permissions affect only that specific command.
- **App-level permissions** are set up by an admin for a specific *app* in their server. These permissions affect all commands for the app (by default).
- **`default_member_permissions`** are set up by an app when creating or modifying a command, and apply to that command in *all* servers unless another override exists. More information about `default_member_permissions` is [in the documentation](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/application-command-permissions-object-using-default-permissions).

The concepts of these three types of permission configurations are not changing. But then of course, the question becomes…
...
We'll go deeper into both of these.
...
With this current system, there is a clear hierarchy: command-level permission configurations take precedence (if present), then application-level permission configurations (if present), then finally `default_member_permissions` if neither are present.

The implication of the current permissions system means that:
- If any command-level permissions are configured, all application-level permissions and `default_member_permissions` are ignored for that command.
- If any application-level permissions are configured, `default_member_permissions` is ignored for all of that app’s commands.
...
Admins will have to opt-in to these changes at first, so you will encounter some guilds with the old behavior, and some with the new behavior described here (starting on December 16). More information about the timing and rollout are below.
...
Starting today, we added a new [`APPLICATION_COMMAND_PERMISSIONS_V2` feature flag](#DOCS_RESOURCES_GUILD/guild-object-guild-features), which indicates whether that server is using **the current permissions logic**.
...
Your app will only be affected if it uses the [`PUT /applications/<application_id>/guilds/<guild_id>/commands/<command_id>/permissions`](#DOCS_INTERACTIONS_APPLICATION_COMMANDS/edit-application-command-permissions) endpoint. This is a pretty restricted endpoint that is used to manage and update application command permissions on behalf of moderators, meaning that it requires the `applications.commands.permissions.update` scope.
...
Use this flag to determine which permissions logic the current guild is using. While the transition from the old behavior to the new behavior is happening, you may need two code paths depending on if the flag is present or not.

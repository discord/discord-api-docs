...
- [Escopos OAuth2](/docs/topics/OAuth2#oauth2-scopes) determinam quais dados e quais ações seu app pode acessar e realizar, concedidos em nome de um usuário que esteja instalando ou autenticando o app.
...
Leia a documentação para ver uma lista de todos os [escopos OAuth2](/docs/topics/OAuth2#oauth2-scopes) ou saber mais sobre [permissões](/docs/topics/Permissions).
...
Se quiser personalizar seus comandos ou adicionar novos, você pode consultar a estrutura de comando na [documentação de comandos](/docs/interactions/Application_Commands#application-command-structure).
...
O código acima respondeu à interação com uma mensagem no canal na qual ela se originou. Você pode ver todos os tipos de respostas disponíveis (por exemplo, responder com um modal) [na documentação](/docs/interactions/Receiving_and_Responding#interaction-callback-type).
...
Você pode ler mais sobre opções de comando e a estrutura delas [na documentação](/docs/interactions/Application_Commands#application-command-option-structure).
...
Quando usuários interagem com um componente de mensagem, o Discord envia uma solicitação com um [tipo de interação](/docs/interactions/Receiving_and_Responding#interaction-type) de `3` (ou o valor `MESSAGE_COMPONENT` ao usar `discord-interactions`).
...
O array de `options` é povoado usando o método `getShuffledOptions()` do `game.js`, que manipula os valores de `RPSChoices` para se conformar ao formato das [opções de componente de mensagem](/docs/interactions/Message_Components#select-option-structure).
